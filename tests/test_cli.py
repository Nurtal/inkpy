...
RAT_STRIPS = {
    # strip: what check reports, and the page it renders to
    "same-seed.yaml": ("4 panels, 7 bubbles, 0 warnings", (1400, 1000)),
    "friday-deploy.yaml": ("4 panels, 5 bubbles, 0 warnings", (1600, 460)),
}
...
def test_the_rat_library_carries_its_rooms():
    """The backdrops are drawn rather than cut: no sheet carries any scenery."""
    result = run("assets", "list", str(EXAMPLES / "rats" / "assets"))
    assert result.exit_code == 0, result.output
    assert "backgrounds: office, server-room" in result.output
...
@pytest.mark.parametrize("name", sorted(RAT_STRIPS))
class TestTheRatStrips:
    """The other cast: sprites cut from drawn sheets, not generated by a script.

    Worth its own cases because the library differs in kind. These characters
    have no face layer — the sheets were drawn one emotion at a time, so the
    expression is in the pose rather than in a swappable face.

    Both are asserted to come out silent. An example that warns is an example
    teaching people to skip warnings.
    """

    def test_it_renders_cleanly(self, name):
        result = run("check", str(EXAMPLES / "rats" / name))
        assert result.exit_code == 0, result.output
        assert RAT_STRIPS[name][0] in result.output

    def test_it_produces_a_page(self, name, tmp_path):
        out = tmp_path / "out.png"
        result = run("render", str(EXAMPLES / "rats" / name), "-o", str(out))
        assert result.exit_code == 0, result.output
        with Image.open(out) as image:
            assert image.size == RAT_STRIPS[name][1]

    def test_the_render_verifies_against_its_sources(self, name, tmp_path):
        strip = EXAMPLES / "rats" / name
        out = tmp_path / "out.png"
        run("render", str(strip), "-o", str(out))
        assert run("verify", str(strip), str(out)).exit_code == 0
